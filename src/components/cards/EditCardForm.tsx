...
export default function EditCardForm({ card, onCardUpdated, onCancel }: EditCardFormProps) {
	...
	const [examples, setExamples] = useState<Example[]>(
		Array.isArray(card.examples) && card.examples.length > 0 ? card.examples : [{ sentence: "", translation: "" }],
	);
	...
	return (
		<form
			...
			className="bg-surface rounded-md shadow-sm p-4 sm:p-6 md:p-8 space-y-4 border border-border"
		>
			<CardFormFields
				title="Edit Card"
				allowAi={allowAi}
